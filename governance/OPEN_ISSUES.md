...
| I062 | **🟡 F1 RLS gap — 50+ multi-tenant tables بدون RLS policies** — production VPS query كشفت 11 جدول فقط فيه RLS policy، بينما ~50+ جدول يحوي companyId أو branchId (sales_invoices, journal_entries, audit_logs, customers, employees, products, bank_accounts, إلخ). الحماية الحالية app-level فقط (Prisma queries بـ `where: { companyId }` filter). مخالفة لـ CLAUDE.md F1 ("RLS policy على كل جدول فيه company_id"). الـ infrastructure للـ RLS موجود (RlsInterceptor + setRlsContext في PrismaService)، فقط الـ policies نفسها مفقودة. | 🟡 مهم | Wave 1 | Backend/Security | مفتوح — يحتاج جلسة مخصصة لكتابة migration ينشئ ~50 RLS policy على نمط `USING ("companyId" = current_setting('app.current_company')::char(26))`. خطر: تفعيل RLS على جداول مالية قد يكسر queries لو الـ session vars لم تُضبط (set_config مع is_local=true يحتاج transaction). يستحق RCA منفصل قبل تنفيذ. تأكيد إضافي عبر Phase 3.D smoke tests (Session 36) — 4 endpoint failures all stem from this gap. |
| I063 | **System owner ينُشأ بدون role assignment على greenfield** — `seed-bootstrap.ts` ينشئ user `ahrrfy@al-ruya.iq` لكن ما يربطه بـ `super_admin` role في `user_roles`. النتيجة: owner يسجّل دخول بنجاح، لكن كل authed endpoint يرجع 403. اكتُشف خلال Phase 3.A evidence sweep (Session 36) — تم إصلاحه يدوياً على VPS بـ `INSERT INTO user_roles SELECT u.id, r.id, NOW(), u.id FROM users u, roles r WHERE u.email='ahrrfy@al-ruya.iq' AND r.name='super_admin' ON CONFLICT DO NOTHING`. | 🟡 مهم | Wave 1 | Backend | مفتوح — fold INSERT في `seed-bootstrap.ts` بعد إنشاء owner user (idempotent ON CONFLICT). جلسة قادمة 30min. |
| I064 | **`GET /finance/periods/status` 500 على empty period table** — اكتُشف خلال Phase 3.A evidence sweep (Session 36). الـ service يفترض وجود periods ويرمي 500 لما الـ table فارغ على greenfield. | 🟡 مهم | Wave 4 | Backend | مفتوح — `periods.service.ts` يرجع `{ open: null, locked: [], openCount: 0 }` لما `count = 0` بدل throw. |
| I065 | **`GET /hr/attendance/report/monthly` 500 بسبب params مفقودة** — اكتُشف خلال Phase 3.A (Session 36). الـ endpoint بدون query params (`month`, `year`) يرمي 500 بدل 400 أو استخدام defaults. | 🟡 مهم | Wave 5 | Backend | مفتوح — controller validation: default إلى `{month: now().month, year: now().year}` أو 400 صريح. |
| I066 | **`GET /admin/licensing/analytics/summary` 500 على greenfield** — اكتُشف خلال Phase 3.A (Session 36). على tenant جديد بدون subscriptions/billing data، الخدمة ترمي 500. متوقع: `{ totalRevenue: 0, activeSubscriptions: 0, ... }`. | 🟡 مهم | Wave 6 | Backend | مفتوح — في `analytics.service.ts` guard على `count() === 0` وترجع zeroed DTO. |
