...
|---|---|---|---|---|---|
| I033 | Orchestrator يستخدم `git checkout` على worktree مشترك → عدة جلسات Claude متوازية في نفس المجلد تخرب HEAD/index/working tree لبعضها. شواهد جلسة 10: 8 تبديلات branch قسرية في 90 ثانية + commit cf6a344 المُسمَّى "claim(T33)" يحوي كود T38. الجذر: `task.sh` خطوط 224, 245, 250, 316, 339 كلها `git checkout` بدون git worktree isolation. | 🔴 حرج | Wave 2 | DevOps | 🔄 **إصلاح كامل قيد المراجعة** (2026-04-27، branch `fix/i033-orchestrator-worktree-isolation`) — `cmd_claim` ينشئ worktree معزولاً تحت `.worktrees/<tid>/`، `cmd_complete` و `cmd_release` يستخدمان `git worktree remove` بدل `git checkout`، typechecks تعمل داخل الـ worktree، إضافة `TASK_SINGLE_SESSION_LOCK=1` كقفل اختياري. PR #108 (الحراسة الجزئية في cmd_claim) يبقى. سيُغلق نهائياً عند merge. |
