...
| I052 | **`/api/v1/licensing/me/features` يُرجع 404 من المتصفح** حتى بعد إصلاح I051. الـ endpoint موجود في `me-features.controller.ts` ومُسجَّل في `LicensingModule`، لكن الـ route لا يُستجاب. السبب المحتمل: ترتيب imports في `app.module.ts` أو globalPrefix mismatch. يجب التحقق من `nest start` logs لمعرفة ما إذا كان الـ controller مُحمَّل، وفحص الـ route mapping عبر `GET /api/v1` debug endpoint إن وُجد. هذا الـ 404 لا يكسر الـ navigation (middleware fails-open) لكنه يمنع feature gating الديناميكي في الـ UI. | 🟡 مهم | Wave 1 | Backend | مفتوح — يحتاج تشخيص على VPS بعد deploy I051. |
| I053 | **WebSocket connection failed** — `wss://ibherp.cloud/socket.io/?EIO=4&transport=websocket` يُغلق قبل establishment. nginx route موجود (`bootstrap.conf:143-154` proxies `/socket.io/` → `api:3000` مع upgrade headers). الجذر المحتمل: (1) host nginx vhost لا يُمرّر WebSocket upgrade headers بشكل صحيح، (2) RealtimeGateway لا يقبل origin من `ibherp.cloud`، (3) auth handshake يفشل بسبب الكوكي المُصلَح في I051. النتيجة: "غير متصل · البيانات قد تكون قديمة" في الـ topbar + لا realtime invalidation للـ feature flags / notifications. | 🟡 مهم | Wave 1 | Backend/DevOps | مفتوح — يحتاج فحص `host-vhost-ibherp.conf` لإضافة `proxy_set_header Upgrade` + `proxy_set_header Connection "upgrade"` على `/socket.io/` location specifically. |
| I054 | **`api()` 401 handler يمسح الـ token مباشرة** قبل أن يحاول الـ refresh — لا refresh-token rotation logic. كل request 401 (حتى المؤقت بسبب JWT clock skew) يطرد المستخدم لصفحة الدخول. مع JWT TTL = 15 دقيقة، المستخدم النشط يُطرد كل ربع ساعة. الحل: تطبيق refresh-token flow في `api.ts` — على 401، استخدم `refreshToken` (من login response) لطلب access token جديد قبل اعتبار الجلسة منتهية. | 🟡 مهم | Wave 1 | Frontend | مفتوح — `LoginSuccessResponse.refreshToken` متاح لكن غير مُستخدَم client-side. |
| I055 | **Auth rate limit صارم (10r/m)** — `bootstrap.conf:22` يضع `erp_auth` zone على `10r/m` مع `burst=5 nodelay`. هذا يكفي لتسجيل دخول نظيف لكن يفشل عند: (1) المستخدم يُجرب كلمة مرور خاطئة عدة مرات، (2) `apiMe()` يُستدعى متكرراً في sessions متعددة، (3) refresh-token flow (I054) يُفعَّل مستقبلاً. يُفضَّل رفع الحد لـ `30r/m burst=10` وفصل `/auth/refresh` عن `/auth/login` بـ rate limits منفصلة. | 🟢 تحسين | Wave 1 | DevOps | مفتوح |
