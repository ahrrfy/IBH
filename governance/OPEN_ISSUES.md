...
| I058 | **🔴 Prisma 7 enum case mismatch — `/licensing/me/features` 500 في الإنتاج** — اكتُشف أثناء الـ deploy verification لـ I052 (2026-04-29 17:06 UTC، VPS SSH session). الـ route مُسجَّل بنجاح بعد I052 (mirror module loaded)، لكن `MeFeaturesController → FeatureCacheService.get(companyId)` يفشل بـ `DriverAdapterError: type "public.SubscriptionStatus" does not exist` عند query subscriptions. الجذر: PostgreSQL يحوي ENUM type باسم `subscription_status` (snake_case، من migration قديمة)، لكن `apps/api/prisma/schema.prisma` يُعرّف `enum SubscriptionStatus` بدون `@@map`. Prisma 7 driver-adapter (`@prisma/adapter-pg@7.8.0`) يصدر SQL casts كـ `"public"."SubscriptionStatus"` (PascalCase، quoted) فلا يطابق. **إضافياً**: CLAUDE.md F2 ينصّ على `❌ لا تستخدم ENUM في PostgreSQL — استخدم VARCHAR + CHECK constraint`. الحل المُفضَّل: إزالة الـ enums واستبدالها بـ VARCHAR + CHECK. الحل المؤقت: إضافة `@map`/`@@map` لكل enum يطابق snake_case الفعلي في DB. أثر الإنتاج: feature gating الديناميكي معطّل (الـ UI يخفي وحدات gated مثل HR إذا plan غير محمّل)؛ I051-I055 لا تتأثر، الـ navigation يعمل. | 🟡 مهم | Wave 1 | Backend | مفتوح — يحتاج schema migration. كشف الفحص: غير `SubscriptionStatus` فقط أم `BillingCycle` و enums أخرى تحتاج نفس الإصلاح. |
