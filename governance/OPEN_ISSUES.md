...
| I029 | Deploy يفشل لأن `WHATSAPP_PHONE_ID` غير موجود في VPS `.env` — `:?` في compose يوقف كل build | 🔴 حرج | Wave 1 | DevOps | ✅ **مغلق** (2026-04-26) — whatsapp-bridge نُقل لـ `profiles: [whatsapp]` في docker-compose.bootstrap.yml. deploy-on-vps.sh لا يبني whatsapp-bridge في الـ deploy العادي. التفعيل اليدوي بعد إضافة credentials: `docker compose --profile whatsapp up -d whatsapp-bridge` |
