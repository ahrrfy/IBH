...
export default function VendorInvoiceDetailPage() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => api<any>(`/purchases/invoices/${id}`),
    enabled: !!id,
  });

  if (isLoading) return <div className="p-6 text-slate-500">جارٍ التحميل…</div>;
  if (error || !data) return <div className="p-6 text-rose-600">تعذَّر تحميل فاتورة المورد</div>;
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-start justify-between">
        <div>
          <Link href="/purchases/invoices" className="text-sm text-sky-700 hover:underline">← العودة للقائمة</Link>
          <h1 className="mt-2 text-3xl font-bold">فاتورة مورد {data.number}</h1>
          <p className="text-sm text-slate-500">{formatDate(data.invoiceDate)} · {data.supplier?.nameAr ?? '—'}</p>
        </div>
