...
export default function NewSupplierPage() {
  ...
  async function submit() {
    ...
    try {
      const created = await api<any>('/purchases/suppliers', { method: 'POST', body: form });
      router.push(`/purchases/suppliers/${created.id}`);
    } catch (e: any) {
      setErr(e?.messageAr ?? 'تعذَّر إنشاء المورد');
  ...
  return (
    <div className="space-y-6">
      <header>
        ...
        <h1 className="mt-2 text-3xl font-bold">مورد جديد</h1>
      </header>
      ...
      <section className="grid gap-3 rounded-lg bg-white p-4 shadow-sm md:grid-cols-2">
        ...
        {field('paymentTermDays', 'مدة الدفع (يوم)', 'number')}
      </section>

      {err && <div className="rounded bg-rose-50 p-3 text-rose-700">{err}</div>}

      <div className="flex justify-end gap-2">
        <Link href="/purchases/suppliers" className="rounded border px-4 py-2">إلغاء</Link>
